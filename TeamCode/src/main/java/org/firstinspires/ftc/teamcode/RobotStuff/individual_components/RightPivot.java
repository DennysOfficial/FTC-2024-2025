...
@Config
public class RightPivot extends ControlAxis { //schrödinger's code

    RightLift lift;

    public void assignLift(RightLift lift) {
        if (lift == null)
    ...
    @Override
    float getInput() {
        return (config.playerTwo == null) ? 0 : config.playerTwo.pivotAxis.getValue();
    }
