...
@Config
public class PassiveGrabberGroup {
    ...
    public PassiveGrabberGroup(OpMode opmode, RobotConfig config, armGroup arm) {
        this.opmode = opmode;
        this.config = config;
        this.arm = arm;
    ...
    public void Score() {
        if (reverseScore) {
            ...
            arm.fancyMoveArm(liftPosReverseScore, 0.5, pivotPosReverseScore, 0.5);
        } else {
            wrist.setPosition(wristPosScore);
            elbow.setPosition(elbowPosScore);
            arm.fancyMoveArm(liftPosScore, 0.5, pivotPosScore, 0.5);
        }
    }
    ...
    public void Collect() {
        ...
        arm.fancyMoveArm(liftPosCollect, 0.5, pivotPosCollect, 0.5);
    }

    public void Rest() {
        wrist.setPosition(wristPosRest);
        elbow.setPosition(elbowPosRest);
        arm.fancyMoveArm(liftPosRest, 1, pivotPosRest, 1);
    }
}
