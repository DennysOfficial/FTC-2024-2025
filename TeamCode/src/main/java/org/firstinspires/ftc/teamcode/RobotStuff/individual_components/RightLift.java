...
@Config
public class RightLift extends ControlAxis { //schrödinger's code

    RightPivot pivot;

    public void assignPivot(RightPivot pivot) {
        if (pivot == null)
    ...
    @Override
    float getInput() {
        return (config.playerTwo == null) ? 0 : config.playerTwo.liftAxis.getValue();
    }
